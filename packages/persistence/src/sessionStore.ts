/**
 * Conversations that outlive the process (SESSIONS.md §9).
 *
 ...
 *
 * The seam for keeping it has existed the whole time: `sessionServicesFor(bundle, summarize, {inner})`
 * takes the base store and has never been passed one. This is that store.
 *
 * ## What it is NOT
 *
 * Not a message table. A session IS the records sharing a `session.id`, ordered by `seq` — upstream's
 * design, and the reason a record holds the whole `LlmOutput` verbatim. Splitting messages out would
 * mean deciding, here, which parts of a provider's answer are worth keeping.
 *
 * ## The two rules that make the lineage work
 *
 * **A position is claimed by the record occupying it.** `open` inserts, and the primary key is what
 * refuses a second claim — so "someone got there first" is a constraint violation rather than a check
 * with a race in it. The caller answers by FORKING, never by retrying at the next slot: appending at
 * 15 instead of 14 continues a conversation containing a turn the call never saw.
 *
...
export class SqliteSessionStore implements SessionStore<JsonValue>, RecordStore {
  ...
  open(stub: RecordStub): void {
    ...
    try {
      this.db
        .prepare(
          `INSERT INTO operation_records (session_id, seq, record_id, task_id, run_id, started_at)
           VALUES (?, ?, ?, ?, ?, ?)`,
        )
        .run(this.k(at.id), at.seq, stub.id, this.scope.taskId ?? null, this.scope.runId ?? null, stub.startMs ?? Date.now());
    } catch (e) {
  ...
  close(id: string, settled: Pick<StoredRecord, "result" | "metrics" | "sessionOutcome">): void {
    ...
    // An executor whose payload IS a conversation needs nothing further; one whose payload is not — a
    // delegated agent, a fake — reports its delta on the session channel, and that is what the
    // conversation is made of. Preferring it keeps the read below looking at one shape.
    // Stored in upstream's own record shape — `{ value: … }`, with the messages inside it — because
    // `defaultMessagesOf` reads `record.result.value.messages` and `bySession` hands these back to it.
    //
    ...
    // bare `{ messages }` would discard exactly what record mode was turned on to keep. The report is
    // for a payload that is NOT a conversation: a delegated agent answering with text, a value-mode
    // prompt core whose payload was projected away inside the call, a scripted fake.
    const reported = settled.sessionOutcome?.messages;
    const payload = settled.result as { value?: { messages?: unknown } } | undefined;
    const value = (payload?.value?.messages !== undefined
      ? settled.result
      : reported !== undefined
        ? { value: { messages: reported } }
        : settled.result) as JsonValue | undefined;
    this.db
      .prepare(
        `UPDATE operation_records
            SET result_json = ?, metrics_json = ?, external_id = COALESCE(?, external_id), ended_at = ?
  ...
  private head(id: string): number {
    const max = this.db
      .prepare(`SELECT MAX(seq) AS m FROM operation_records WHERE session_id = ?`)
  ...
  private derive(ref: string, word: string, messages: readonly JsonValue[]): string {
    ...
    this.db
      .prepare(
        `INSERT OR REPLACE INTO operation_records (session_id, seq, record_id, task_id, run_id, result_json, started_at, ended_at)
         VALUES (?, 0, ?, ?, ?, ?, ?, ?)`,
      )
      .run(
        this.k(derived),
  ...
  private rowsOf(session: string, upTo?: number): Row[] {
    const rows = this.db
      .prepare(
        `SELECT seq, record_id, result_json, external_id FROM operation_records
          WHERE session_id = ? ${upTo === undefined ? "" : "AND seq < ?"} ORDER BY seq`,
      )
    ...
    return rows.map((row) => ({
      ...
      ...(row.result_json !== null ? { value: JSON.parse(row.result_json) as JsonValue } : {}),
