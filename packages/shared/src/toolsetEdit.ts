...
//
// "Add to the toolset" writes a line. Settings → Toolsets writes a TOOLSET: lines added, lines
 ...
 */
export function overridesOf(base: ToolsetDecl, next: ToolsetDecl): { siblings: ToolsetDecl; dropped: string[] } {
  const siblings: ToolsetDecl = {};
  for (const [subject, entry] of Object.entries(next)) {
    if (!sameToolsetEntry(subject, Object.hasOwn(base, subject) ? base[subject] : undefined, entry)) siblings[subject] = entry;
  }
  const dropped = Object.keys(base).filter((subject) => subject !== OTHER_SUBJECT && !Object.hasOwn(next, subject));
  return { siblings, dropped };
}

/** A NEW toolset file: the map, one line per subject, `$ref` first when it starts from another. */
export function newToolsetText(next: ToolsetDecl, follows?: string): string {
  if (follows !== undefined && !/^\$[A-Z]+\//.test(follows)) {
    throw new Error(`an override follows an explicit root ('$SYSTEM/…', '$BASE/…') — '${follows}' would find the override itself`);
  }
  const lines = [
    ...(follows !== undefined ? [`  ${JSON.stringify(TOOLSET_REF_KEY)}: ${JSON.stringify(follows)}`] : []),
 ...
 * `other`, which a person reads as the last line of a toolset and which therefore stays it. A `$ref`
 ...
 */
export function setToolsetText(text: string, next: ToolsetDecl, follows?: string): string {
  const errors: ParseError[] = [];
  const parsed: unknown = parseJsonc(text, errors, { allowTrailingComma: true });
  if (errors.length > 0) throw new Error("the toolset file does not parse as JSON, so nothing was written to it");
  if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("the toolset file is not a map from a subject to a mode, so it cannot be edited line by line");
  }
  const wanted = new Map<string, ToolsetEntryDecl>(Object.entries(next).map(([subject, entry]) => [normalize(subject), entry]));
  ...
  for (let index = (tree(out).children ?? []).length - 1; index >= 0; index--) {
    ...
    if (!readable || !sameToolsetEntry(subject, held as ToolsetEntryDecl, entry)) out = replace(entryText(entry));
  }
  ...
  if (follows !== undefined && !has(TOOLSET_REF_KEY)) {
    const root = tree(out);
    out = (root.children ?? []).length > 0 ? insertPropertyBefore(out, root, 0, TOOLSET_REF_KEY, JSON.stringify(follows)) : newToolsetText({}, follows);
  }
