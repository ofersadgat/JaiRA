/**
 * An agent gets its permission set and nothing else (decision 0007 §3) — through the REAL wiring.
 *
 * The chain under test is the one `agentPolicy.test.ts` pins, one step further: `executeWorkflow` →
 * the engine (which resolves a state's tools and builds its gate) → `withAgentPermissionSet` (JaiRA's
 * wrapper, holding the route's executor to the permission set by that executor's own declaration) → the
 ...
 *
 *  - NOT in the permission set → the native is removed;
 ...
 *
 * ALL OF THAT IS WHAT A MAP SAYS. A state that declares NO permission set has said nothing about tools, and
...
import { lowerStatePermissionSets, parsePermissionSet } from "@jaira/shared";
import { CODEX_TOOLS, planAgentTools, withPermissionSetService } from "../src/agentTools";
...
async function run(
  ...
  const lowered = lowerStatePermissionSets("digest", def, () => {
...
/** Everything about a spawn a permission set could have changed. */
...
describe("a state that declares NO permission set runs exactly as the bare executor did (c0b5972)", () => {
...
describe("a read-only permission set MAP reaches claude holding everything `profile: \"read-only\"` removed", () => {
  ...
  it("removes MORE than the profile did: the built-in of every standard tool the permission set does not hold", async () => {
    // The profile was a fence against writers and said nothing about `Glob`, `Grep` or the web
    // readers, which ran ungoverned beside a list that never granted them. A permission set is the whole
...
describe("a read-only permission set reaches codex as the same sandbox flag", () => {
  it("is `--sandbox read-only` from a map — codex's `plan` is nothing else", async () => {
    ...
    // Codex has no deny list and refuses one by name, so the permission set writes NOTHING into it: what is
  ...
  it("is derived from the PERMISSION_SET: the writing sandbox is on only for a writer of codex's own it keeps", async () => {
    ...
    // …and a permission set that holds only readers leaves it off, with no profile anywhere.
  ...
  it("leaves the writing sandbox OFF for a shell the permission set denies — in a RUN, as in a conversation turn", async () => {
    ...
    // …which is what a conversation turn, holding the permission set itself, derives.
    for (const decl of [withheld, commandsOnly]) {
      expect(planAgentTools(parsePermissionSet(decl).permissionSet, CODEX_TOOLS).switches).toEqual({ "workspace-write": false });
    }
    ...
    expect(planAgentTools(parsePermissionSet(kept).permissionSet, CODEX_TOOLS).switches).toEqual({ "workspace-write": true });
  });
});

describe("a shell the permission set denies outright is WITHHELD, in a run", () => {
...
describe("the postmortem case: tools with no restriction (task t-19fipjbavj)", () => {
  it("a MAP does not leave the CLI agent its own Edit — the map is the whole grant", async () => {
    ...
    // disk. As a map no profile is needed: `edit` is not in the permission set.
  ...
  it("a state that declares NO permission set never runs Edit UNGATED: the call reaches the gate, and a person", async () => {
...
describe("nothing is granted outside the permission set", () => {
...
describe("a transport that enforces nothing", () => {
  ...
  it("still refuses a permission set that refuses anything — by name, with no profile to go by", async () => {
    ...
    expect(JSON.stringify(result)).toMatch(/aider: this state's permission set refuses .*'edit', 'write_file'.*'other', and this transport enforces nothing/);
  });
...
describe("the implementation choice — a RUN, which reads it off the state's block", () => {
  ...
  it("holds a run with NO approver to its map too — the block is published without a gate", async () => {
    ...
    const { seen, query } = capturingQuery();
    const lowered = lowerStatePermissionSets(
      "digest",
      {
        label: "Read and report",
        outputs: { report: { kind: "text", schema: { type: "string" } } },
...
describe("an agent reached as a FUNCTION is held to its permission set — every channel, not only claude's", () => {
  async function runFunction(
    ...
    const lowered = lowerStatePermissionSets("delegate", def, () => {
      throw new Error("no reference expected");
    });
    const reg = registry();
    register(reg, query);
  ...
  it("codex-cli: a permission set that leaves the writing switch off runs read-only, over whatever the call itself asked for", async () => {
    ...
    // …and a switch the permission set leaves on — codex's own shell kept — keeps the call's own mode…
    ...
    // …and a state that declares no permission set is handed on exactly as the engine built it.
  ...
  it("a generic CLI refuses a permission set that refuses anything, by name — as its prompt route does", async () => {
...
describe("the implementation choice — a conversation turn, which knows its permission set", () => {
  it("keeps a `native` built-in, forces it to the callback, and lets the ENTRY's mode decide", async () => {
    const { permissionSet } = parsePermissionSet({ read_file: "allow", grep: { mode: "ask", implementation: "native" }, other: "deny" });
    const plan = planAgentTools(permissionSet);
    const { seen, query } = capturingQuery();
    const asked: string[] = [];
    ...
    // What `runChatMessage` builds: ours wrapped and gated, and the permission set published for the executor.
    const { tools, gate } = gateTools({ registry: registry(), names: plan.inject.filter((name) => name !== "show_artifact"), sessionId: "s1", approve, permissionSet });
    const services = withPermissionSetService({ tools, gate, approve } as ExecServices, permissionSet);
    ...
    expect(asked).toEqual(["grep"]);
    // A tool the permission set does not hold was removed, and would be refused unasked if it turned up.
