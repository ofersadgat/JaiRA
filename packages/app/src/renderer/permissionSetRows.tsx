/**
 * The lines a PERMISSION_SET is drawn as — one set of rows, wherever a permission set is on screen.
 *
 * They were the composer's, inside `composer.tsx`. Settings → Permission sets draws a permission set "as the
 ...
 *    unticked one keeps its row. In a permission set being edited the card lists what the permission set HOLDS, so
...
export function modeMeta(mode: PermissionSetMode): { icon: Parameters<typeof Icon>[0]["name"]; label: string; hint: string } {
...
const modeClass = (mode: PermissionSetMode): string => (isFunctionMode(mode) ? "function" : mode);

/**
 * The red minus that starts a held line in a permission set being edited: out of the permission set.
 *
 ...
 *
 * With `held` the row is a line of a permission set being EDITED or READ rather than ticked: no tick, the
 * name is text and not a button, and `onRemove` — when the permission set can be changed — draws the minus.
 */
export function ToolRow({
  ...
  /** A line of a permission set on the page, not a tick box — see above. */
  held?: boolean | undefined;
  /** Said under the name INSTEAD of the tool's own sentence: "over the permission set, for this state". */
  ...
  // permission set on the page has no one agent answering: the choice is drawn wherever ANY agent has a
...
export function CategoryRow({
  ...
  onOpen: () => void;
  /**
   * Set every line under it. Absent ⇒ no button: a permission set on the page lists only what it HOLDS, and
   * a head that re-moded those lines would be a second way to do what each line's own button does.
   */
  onMode?: ((next: PermissionSetMode) => void) | undefined;
}): JSX.Element {
  return (
...
/**
 * A line of Execution that is not a tool: a command the permission set names (`git commit`), or `script`.
 *
...
export function SubjectRow({
  ...
  onMode: (next: PermissionSetMode) => void;
  /** A line of a permission set on the page, not a tick box — see {@link ToolRow}. */
...
/**
 * The last line of a section or a group in a permission set being edited: "add a tool", "add a git
      ...
      <div className="cx-cat-head">
        ...
        <button type="button" className="cx-cat-fold" aria-expanded={open} onClick={onOpen}>
          ...
          <span className="cx-opt-text">
            ...
            <span className="cx-opt-hint ellip">{open ? groupSentence(permissionSet, group) : groupSummary(permissionSet, group)}</span>
          </span>
          {group.subs.length > 0 ? <span className="cx-cat-count">{group.subs.length}</span> : null}
        </button>
        <ModePicker
          mode={groupModeOf(permissionSet, group)}
