/**
 * A toolset ON THE PAGE — the composer's Tools card, standing still (decision 0007 §6).
 *
 * Two render functions over the rows of `toolsetRows.tsx` and the edits of `composerToolset.ts`:
 *
 *  - {@link ToolsetCard} is the whole card, the way the composer draws it — the sections, a line per
 *    subject with its implementation and its mode, commands grouped under their program, `Other`
 *    last — for Settings → Toolsets;
 *  - {@link ToolsetLines} is the same rows with no sections, for the few lines a STATE writes over
 *    the toolset it starts from.
 *
 ...
 * a toolset on the page lists what it HOLDS, because it is a file, and absent is how a file says not
 ...
 *
 * Neither holds anything but which folds are open. The toolset arrives, and the next one leaves
 * through `onChange`, whole: a toolset is one statement.
 */
...
import { AddMenu, CategoryRow, CommandGroupRow, ModePicker, SCRIPT_HINT, SHELL_HINT, SubjectRow, TOOL_ICONS, ToolRow, type AddOption } from "./toolsetRows";
...
/** The sections a toolset opens on: the ones that hold a line. An empty one is a heading and a count of nothing. */
export function openSectionsOf(toolset: Toolset): string[] {
  return TOOL_CATEGORIES.filter((category) => sectionCountOf(toolset, category.id) > 0).map((category) => category.id);
}

export interface ToolsetCardProps {
  toolset: Toolset;
  /** Every tool a line can be written for, with what each agent calls its own. */
  tools: readonly ToolChoice[];
  /** The next toolset, whole. Absent with `readOnly`. */
  onChange?: ((next: Toolset) => void) | undefined;
...
export function ToolsetCard({ toolset, tools, onChange, readOnly, folds, onFold, startAdding, startMode }: ToolsetCardProps): JSX.Element {
  ...
  const write = (next: Toolset): void => onChange?.(next);
  ...
  return (
    <div className={`cx-cats${locked ? " set-readonly" : ""}`}>
      {TOOL_CATEGORIES.map((category) => {
        ...
        const held = inCategory.filter((tool) => holdsTool(toolset, tool.name));
        const groups = category.id === "execution" ? commandGroupsOf(toolset) : [];
        const script = category.id === "execution" && Object.hasOwn(toolset.entries, SCRIPT_SUBJECT);
        ...
        return (
          <CategoryRow
            key={category.id}
            category={category}
            granted={sectionCountOf(toolset, category.id)}
          ...
          >
            {held.map((tool) => (
              <ToolRow
                ...
                granted
                mode={toolModeOf(toolset, NO_PARKED, tool.name)}
                impl={toolImplementationOf(toolset, NO_PARKED, tool.name)}
                readOnly={locked}
            ...
            {groups.map((group) => (
              <CommandGroupRow
                ...
                readOnly={locked}
                onRemove={locked ? undefined : () => write(withoutProgram(toolset, group.program))}
                onMode={(next) => write(withSubject(toolset, group.program, next))}
              >
                {group.subs.map((sub) => (
                  <SubjectRow
                    key={sub.subject}
                    ...
                    mode={sub.mode}
                    readOnly={locked}
                    onRemove={locked ? undefined : () => write(withoutSubject(toolset, sub.subject))}
                    onMode={(next) => write(withSubjectMode(toolset, sub.subject, next))}
                  />
                ))}
                {locked ? null : (
                  <AddMenu
                    ...
                    options={suggestedSubcommands(toolset, group.program).map((subject) => ({ subject, label: subject, icon: "terminal" as const, mono: true }))}
                    onPick={(subject) => write(withCommand(toolset, subject))}
                    typed={{
                      label: `add a ${group.program} subcommand`,
                      example: "push",
                      onAdd: (typed) => {
                        ...
                        write(withCommand(toolset, named.subject));
                        return undefined;
                      },
                    }}
                  />
            ...
            {script ? (
              <SubjectRow
                ...
                hint={SCRIPT_HINT}
                held
                mode={toolset.entries[SCRIPT_SUBJECT]?.mode ?? MODE_WHEN_UNSET}
                readOnly={locked}
                onRemove={locked ? undefined : () => write(withoutSubject(toolset, SCRIPT_SUBJECT))}
                onMode={(next) => write(withSubjectMode(toolset, SCRIPT_SUBJECT, next))}
              />
            ) : null}
            {locked ? null : category.id === "execution" ? (
              <AddMenu
                ...
                options={[
                  ...addableTools(toolset, category.id, offered).map(toolOption),
                  ...(addableScript(toolset) ? [SCRIPT_OPTION] : []),
                  ...suggestedPrograms(toolset).map((program) => ({
                    subject: program,
                    label: program,
                ...
                onPick={(subject) => {
                  if (subject === SCRIPT_SUBJECT) write(withSubject(toolset, SCRIPT_SUBJECT, MODE_WHEN_UNSET));
                  else if (TOOL_SPEC_BY_NAME.has(subject)) write(withToolHeld(toolset, NO_PARKED, subject, true));
                  else {
                    write(withCommand(toolset, subject));
              ...
              <AddMenu
                label="add a tool"
                startOpen={startAdding === category.id}
                options={addableTools(toolset, category.id, offered).map(toolOption)}
                onPick={(subject) => write(withToolHeld(toolset, NO_PARKED, subject, true))}
              />
            )}
          </CategoryRow>
        );
      ...
      <div className="cx-cat cx-cat-other">
        <div className="cx-cat-head">
          ...
          <ModePicker mode={toolset.other ?? MODE_WHEN_UNSET} title="Anything this toolset has no line for" readOnly={locked} onMode={(next) => write(withOther(toolset, next))} />
        </div>
      </div>
    </div>
  );
...
export interface ToolsetLinesProps {
  /**
   * The lines, as a toolset. `other` is a line here like any other — present only when written —
   * which is why it arrives beside the toolset rather than inside it: a `Toolset` cannot say whether
...
/**
 * The same rows, flat: a few lines with no sections — what a state writes over its toolset.
 *
 ...
 */
export function ToolsetLines({ toolset, tools, onChange, readOnly, note, addLabel, startAdding }: ToolsetLinesProps): JSX.Element {
  const locked = readOnly === true || onChange === undefined;
  const write = (next: Toolset): void => onChange?.(next);
  const byName = new Map(tools.map((tool) => [tool.name, tool]));
  const clearOther = (): void => {
    const { other: _gone, ...rest } = toolset;
    write(rest);
  };
  return (
    <div className={`set-card${locked ? " set-readonly" : ""}`}>
      <div className="cx-cat-body set-flat">
        {Object.entries(toolset.entries).map(([subject, entry]) =>
          entry.kind === "tool" ? (
            <ToolRow
              key={subject}
              held
              ...
              readOnly={locked}
              onRemove={locked ? undefined : () => write(withoutSubject(toolset, subject))}
              onMode={(next) => write(withSubjectMode(toolset, subject, next))}
              onImpl={(next) => write(withToolImplementation(toolset, subject, next))}
            />
          ) : (
            <SubjectRow
              key={subject}
        ...
        {toolset.other !== undefined ? (
          <SubjectRow
            ...
            onMode={(next) => write(withOther(toolset, next))}
          />
        ) : null}
        {locked ? null : (
          <AddMenu
            ...
            options={[
              ...tools.filter((tool) => !Object.hasOwn(toolset.entries, tool.name)).map((tool) => toolOption(tool.name)),
              ...(addableScript(toolset) ? [SCRIPT_OPTION] : []),
              ...(toolset.other === undefined ? [{ subject: OTHER_SUBJECT, label: "other", hint: "anything no line names", icon: "shield" as const }] : []),
            ]}
            onPick={(subject) => {
              if (subject === OTHER_SUBJECT) write(withOther(toolset, MODE_WHEN_UNSET));
              else if (subject === SCRIPT_SUBJECT) write(withSubject(toolset, SCRIPT_SUBJECT, MODE_WHEN_UNSET));
              else write(withToolHeld(toolset, NO_PARKED, subject, true));
            }}
            typed={{
              label: "add a command",
              example: "git commit",
