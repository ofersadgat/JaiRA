...
| **API-key routes** | the call's reported usage, against the model table's window | money, not windows — see "API keys" below: what each call cost (`costUsd`), and OpenRouter's credit | OpenRouter only: `GET /api/v1/credits` (management key) or `GET /api/v1/key` (the key's own limit) |
...
Settled with the person over rounds 7 to 13 of the same mockups (2026-09-24). An API key has no window: its allowance is the credit on the account, and JaiRA keeps no budget of its own. What the providers say, checked 2026-09-24:

| | The credit | An empty balance |
| --- | --- | --- |
| **OpenRouter** | `GET /api/v1/credits` → `total_credits`, `total_usage` (a MANAGEMENT key only); an ordinary key reads its own spending limit from `GET /api/v1/key` (`limit`, `limit_remaining`) | HTTP 402 |
| **Anthropic** | not readable: no API returns it, admin key included (the Admin API's cost and usage reports say what was spent). The Console's own `/api/organizations/{org}/prepaid/credits` answers only a signed-in Console browser session — MEASURED with an API key: 403 `account_session_invalid` | HTTP 400 "Your credit balance is too low…" |
| **OpenAI** | not readable (the costs and usage endpoints only) | HTTP 429 with `insufficient_quota` — a 429 that is not a rate limit |

So an account is drawn one of three ways (`UsageCase`):

1. **A subscription** (claude, codex) — the percent of the tightest window, as below.
2. **A credit** (OpenRouter, when its credit could be read) — what is SPENT, in dollars (`$11.60`), coloured by the share of the credit that is. Never what is left. At 100% messages wait, the same as a spent subscription, and are looked at again every half hour, because credit comes back when somebody adds it rather than at a reset.
3. **Spend** (Anthropic, OpenAI, and OpenRouter when its credit cannot be read) — what THIS CONVERSATION has cost, grey, because it is a cost and not a warning. Its ring is that cost's share of what JaiRA's calls on the key spent in the last seven days, ROLLING. It turns red only when a call is refused for an empty balance, and stays red until a call goes through.

## What the screens show

Settled with the person over thirteen rounds of mockups (https://claude.ai/artifact/MCjDTEjMLkBjL3xL6LTXk1, 2026-09-23 to 24), and built as described.
...
- **Composer, after the model chip: the account's figure.** For a subscription, the percent used of the tightest window that applies to the chosen model; for an API key, the money figure of its case ("API keys" above). An account's percent is ink at rest, amber from **75%** and red from **90%** (the person's cut-offs); red is a warning, and only 100% holds messages. Clicking it opens the account: for a money account, first what this conversation has cost; then every window with when it resets — or the credit used, or the key's last seven days — the reading's age and source, Refresh when a refresh is offered, and the person's other accounts, each in its own unit.
- **Appearance → Conversation → Usage figures** (`appearance.conversation.usageFigures`: `off` | `number` | `ring` | `both`, default `number`) says how that figure is drawn: the number, the ring that draws the same share (with a $ in it for money), both, or nothing. A ring beside the percent it draws said one thing twice. It governs the figure after the chip and the Connections cards, and it has a preview; the conversation's own ring, at the composer's right, is not this setting.
...
- **Settings → Connections: each sign-in card is a fixed 200px.** Line one: the icon, the email, and Log out as an icon. Line two: the plan, then the **weekly** window's percent (Claude: `seven_day`, all models; Codex: the secondary window), drawn as Usage figures says — the number alone by default. An API key's box shows what it has spent — the credit used, or the last seven days with "no balance reported" under it. Line three, right-aligned: "Weekly · resets Mon 09:00", red when the week is used up. Clicking the ring opens the same account popover as the composer. If the 5-hour window is spent but the week is not, the card does not change; the composer says so.
- **The model menu** puts the account's figure on each route row: a subscription's tightest-window percent, a credit's dollars used beside the same bar, a spend key's last seven days in grey with no bar. A model row shows the same bar and number, with no words, only when a window counts that model alone (Opus weekly). Presets pick the candidate with the most left.
...
- **An API call's cost reaches the board through the prompt router**, not the executor. `PromptRouterExecutor.start` is the one place that knows the route of every prompt call, agent or provider, so it reads each settled result's `costUsd` and tells `ctx.usage.spent(route, cost)` (upstream `UsageReporter.spent`, optional); `withSessionPosition({ …, spent })` hands it to the host by account. The app keeps it per account for seven days in `system/limits.json` (`spend`, beside `credit` and `refused`).
- **A refusal for an empty balance is `credit_exhausted`** (upstream `CREDIT_EXHAUSTED_CODE`, `isCreditExhausted`), classified `out-of-credits` and asked BEFORE the rate-limit check, so OpenAI's 429 is not retried. It carries `detail.route`. JaiRA marks the account (`creditRefusedAt`) and keeps the message or run as refused with no "Try again at …" (`WaitingItem.credit`).
- **A credit is a board reading too**: one window, `Credit`, no reset — so spent, the tightest window and `most-left` read a credit the way they read a subscription. The dollars ride beside it on the account view (`LimitAccountView.credit`), with `kind` and `spent7d`.
- **Not built:** Anthropic's credit, which only a Console browser session can read; readings from an API-key route's rate-limit response headers; and `model_limits` for a `$pick` expression. The preset rule `most-left` is the consumer that exists.
