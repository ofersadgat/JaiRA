...
| `permissions` | `{profile, default, tools, other, scopes}` | no | all optional; modes are `allow`, `deny`, `ask` or `smart`; `tools` maps a tool name to a mode; `other` covers a tool outside the vocabulary; `scopes` is a `Scope` array |
