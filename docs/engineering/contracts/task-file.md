...
| `connectUndo` | `{undo: ConnectUndo, rows: number, kept: number, landing: string[], asking?: true}` | no | the Undo of the connect that last made or moved this task, kept so it survives a restart, and meaning only "take back what I just did". Counts are journal rows, because a seq does not outlive a replay: `rows` is a move's `after` as the moved task's rows at or before it (for an adoption, the ADOPTED task's rows at the drop); `kept` is the watched journal's rows — the moved task's, or the parent's — when it was written, the drop's own rows included; `landing` is `ConnectPlan.standsAt.path`; `asking` marks a drop that opened a conversation to ask. Written by `task:connect` (replacing an earlier one); removed by the next decision about the task — a connect or adoption of it, a rewind, fork or split, a stop, a re-run — and by `task:connectUndo`. Judged against the journal on every read, so a token the task has moved on from is neither offered nor used ([task-channels](task-channels.md)) |
