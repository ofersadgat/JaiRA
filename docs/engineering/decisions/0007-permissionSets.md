---
id: engineering/decisions/0007-permissionSets
type: decision
status: built
updated: 2026-09-23
...
# 0007. A permission set is the permission model: subject → mode, in buckets; one shell line is several requests
...
permission sets and permission sets — and every example needed one of each:
...
| Option | For | Against |
| --- | --- | --- |
| A. Permission sets and permission sets as two named fragments (0006 as first written) | Mirrors the two fields that exist | They only ever travel in pairs; two pickers, two editors, and `profile` needs a name in the UI |
...
| C. One set: subject → mode, as plain data; "read-only" *is* a permission set; `other` covers what is not named (**chosen**) | One concept, one picker, one editor; nothing to derive a meaning from but the map a person can read | A tool registered later falls to `other` until a set names it; the shipped sets have to be kept current |
...
### 1. A permission set is a map from a subject to a mode

```jsonc
// $SYSTEM/permission-sets/chat/read-only.json
{
...
- **A state names one**: `"tools": "$/permission-sets/chat/read-only"` — a bare
  ...
  `{ "$ref": "$/permission-sets/chat/read-only", "write_file": "ask" }`. A
  permission set file starts from another the same way. An inline map is legal.
- `readOnly`, `TOOL_PROFILES`, `permissions.profile` and the presets'
  `modeFor` **go**. "Read-only" is the permission set of that name and nothing
...
### 2. Permission sets live in buckets

```text
permission-sets/chat/ask-first.json        permission-sets/chat_control/ask-first.json
permission-sets/chat/read-only.json        permission-sets/chat_control/read-only.json
permission-sets/chat/auto.json             permission-sets/chat_control/auto.json
permission-sets/chat/full.json             permission-sets/chat_control/full.json
```
...
what ships is `$SYSTEM/permission-sets/**`, and a file at the same path in
`~/.jaira/` or `.jaira/` wins.

`chat/session` names `$/permission-sets/chat/ask-first`; `chat/control` names
`$/permission-sets/chat_control/ask-first`. The four that ship in `chat` are
today's four presets written down as data.

### 3. An agent gets the permission set and nothing else
...
- For a call, per standard tool: **not in the permission set → the native is
...
- An agent is never handed a tool its permission set does not hold.
...
- **The approval shows the line in the colours of its parts**, and under it
  ...
  *for*, and whether the permission set allows it or asks. What joins the parts
  (`&&`, `|`, a space) is dim. **The tint is the whole part; the underline
  is only the words that matched** the permission set's line — `git commit` and not
...
- **One Allow and one Deny**, each with an arrow. The menu asks two things
  ...
  every command of the program (`git`) — which is how a program the permission set
  ...
  run* (the ledger, nothing written), or **add to the permission set** — which
  writes that line (`"git commit": "allow"`, or `"git": "allow"`) into the
  permission set that asked, in this project or for all projects. It is the asking
...
- A line that cannot be taken apart is never allowed without asking, and
  ...
  every permission set. Both are existing invariants.

### 5. The composer keeps both cards

- **Permissions keeps its look and holds permission sets.** Its rows are the
  permission sets of one bucket — for `chat`: ask first, read-only, auto, full
  ...
  A **`+`** in the head keeps the current map as a new permission set in that
...
- **Tools is unchanged**: tick, implementation, mode, per tool and per
  ...
  permission set names **grouped under their program**: `git` is a line that opens
...
- **A label is a match, never a memory.** Permissions reads a permission set's
  name while the map is exactly that permission set's, and `custom` otherwise —
  which is what it does today. Tools counts, as it does today.

### 6. Settings → Permission sets
...
bucket's name and the layer that defines it, then its permission sets, each with a
line of summary and a dot where this layer sets it, then "+ permission set"; "+
bucket" closes it. The detail is the permission set, **drawn as the composer's
...
[ui/surfaces/settings-models](../../ui/surfaces/settings-models.md).
...
**Hard.** The shipped permission sets are a list somebody has to keep current as
...
2. **Executors declare natives** and coarse switches; `planAgentTools`
   ...
   beside the executors and `withAgentPermissionSet` applies the plan at each
...
3. **Shell**: redirects, substitutions and the embedder list in
   ...
   apart with spans, each part judged against the permission set under the rules
   ...
   buttons, and "add to the permission set". What was decided while building:
   - **Which permission set asked** is carried, because nothing downstream of
     lowering knew: lowering writes `permissions.source` beside
     `subjects`, the reference the state named its permission set by, or
   ...
   - **The write**: a layer that holds the permission set gets the line added in
     ...
     `{ "$ref": "$SYSTEM/permission-sets/<bucket>/<name>", "<subject>": "<mode>" }`.
     Only a bare `$/permission-sets/<bucket>/<name>` reference says which file;
     ...
     snapshot.
4. **Buckets** in `$SYSTEM/permission-sets/**` (needs 0006): the four `chat`
   permission sets as data; `chat_control`. *Built 2026-09-21*
   ([tool-policy](../units/tool-policy.md)): eight files under
   `packages/shared/builtin/permission-sets/`. The `chat` four are held by a test
   ...
   category, Tasks & workflows, with the mark `unserved`: a permission set could
   ...
   anything. **The four `chat` permission sets grew the eight lines**, because a
   ...
   `matchPermissionSet`. Two things a file has no place for are kept beside the
   code instead: the shipped permission sets' labels, hints and glyphs, and the
   ...
   `move_task`, `stop_task` and the rest (2026-09-21), and a permission set can
...
5. **The composer**: Permissions rows from the bucket, the bucket picker,
   ...
   both cards read one `PermissionSet` and every edit sends the whole map as
   `ChatSettings.permissionSet`, so a turn a person touched is a map and is held
   to it (§3), where an untouched one still inherits its state's list. The
   plan carries every permission set on the search path, references followed, and
   the bucket to open on. **That bucket is found by match, not read off
   the state**: a loaded state holds what its permission set lowered to, and the
   reference is gone, so the card opens on the first bucket holding a
   permission set the inherited map exactly is, and on `chat` otherwise. `chat/agent`
   still declares a list of three tools, which is no shipped permission set, so a
   new conversation reads `custom` until it names
   `$/permission-sets/chat/ask-first`. A program's mode button is the entry for
   ...
   `+` writes through `permissionSet:save`, which refuses the built-in layer, a
...
6. **Settings → Permission sets.** *Built 2026-09-21*
   ([settings-tools](../../ui/surfaces/settings-tools.md),
   [permissionSet-card](../../ui/components/permissionSet-card.md),
   [tools-field](../../ui/components/tools-field.md)): the rail is the bucket
   hierarchy, the detail is the permission set drawn as the composer's Tools card is,
   and the layer picker gained the third segment 0006 left for it. What the
   build settled:
   - **The rows are the composer's, moved out and not copied.** `permissionSetRows.tsx`
     ...
     the hosts: `held` (the card lists what the permission set HOLDS, so the name is
   ...
   - **A section's mode button is not drawn on a permission set being edited.** It
   ...
   - **What a save IS depends on the layer**, and is chosen by the writer and
     named in the answer (`PermissionSetWriteKind`): `edit` in place, format kept;
   ...
   - **"Override here" writes `{ "$ref": … }` and nothing else.** An override
     ...
     permission set into a layer they own without freezing today's version of it.
   - **Reset is refused where there is nothing below.** Deleting a permission set that
   ...
   - **"Used by" is a best-effort SCAN and says so**: every `workflows/**` file
     ...
     `tools` key naming `…/permission-sets/<id>`. It does not see a relative reference,
     one reached through another permission set's `$ref`, or a state that inherits its
     tools from an ancestor's `environment` — the ancestor is listed, its
     descendants are not.
   - **`PermissionSetLayerFile.decl` is the RESOLVED map**, references followed, which
   ...
   - **Lines over a permission set never write `other`.** `declOfPermissionSet` always does,
     because a whole permission set should say what happens to everything it does not
     name; a state's lines say only what they CHANGE, and a written catch-all
     would override the permission set's own on every state ever opened in the form.
   - **The field's permission sets arrive by context**, read once per project with
     `usedBy: false` — the pane's "used by" scan reads every state file, and the
     field wants the permission sets alone.
7. **Migration**: `"profile": "read-only"` → a reference to a read-only
   permission set; `permissions.tools` / `default` → entries and `other`. A
   ...
   **does**: `handedToClaude` runs one effective block through the engine, the
   ...
   - The dry run for `~/.jaira` is
     ...
     them are the same map, which wants a permission set file of its own. Nothing under
     `~/.jaira` was written.
...
- **A shell the map denies outright is withheld.** `"bash": "deny"` with no
  ...
  what the author wrote and reads back as held, so a permission set still matches
...
- **"No shell but these commands" is judged by the map in a run too.** When a
  ...
  permission set the decision's own §1 example writes.
...
- **`chat/read-only` does not ship `git status`, `git log` or `git diff`.**
  ...
  tell those apart without flag entries nobody has written, and a permission set
  ...
  "add to the permission set"; that map is then safe in a run by the carried key.
...
permission set (`viewOfServices` through `permissionSetOfEnvironment`). Four failure rows
...
- **An agent reached as a function is held to its permission set.**
  ...
  where the permission set leaves the writing switch off, and a generic CLI refuses a
  permission set that denies anything, as their prompt routes do.

`PERMISSION_SET_MARKERS` stay: they are the map/legacy discriminator, now read off the
...
- **The list form of `tools` and the old `permissions` block are refused.**
  ...
  else. `permissionSetOfLegacy`, `applyLegacyProfile`, the frozen
  `LEGACY_NON_READ_ONLY_TOOLS` / `LEGACY_NARROWING_PROFILES`, `PermissionSet.legacy`
  and the rule that a list-form permission set keeps the agent natives it never
...
- **`jaira workflow migrate-toolsets` is removed**, with `permissionSetMigration.ts`,
  `permissionSetMigrateEdit.ts` and `handedDifferences`. `handedToClaude` and
...
- **`PERMISSION_SET_MARKERS` stay, for a different reason.** A map no longer needs
  telling from a list, but it still needs telling from a state that declares
  no permission set at all — which is not the statement "nothing": an empty map
...
state that declares no permission set — which is what the list form got too — and the
...
### Amended 2026-09-22 (closing): a CLI run, and a permission set inside a referenced block
...
- **A run started by `jaira` is governed as an app run is.** Both hosts now
  ...
  once, and the refusal names why and what would allow it: the permission set line
  where a permission set judged the part, the `policy.rules` entry where a built-in or
...
- **A permission set inside a referenced block lowers as one on the state.** An
  `environment`, `operation` or mount `environment` that is itself a reference
  is opened down its chain; its permission set is lowered from the target's file and
...
- **Codex runs a state held to a permission set as claude does.** Measured against the
...
- **A claude agent reached as a function is handed what its route is.** Upstream's
  ...
  each call; JaiRA passes the route's own `withAgentPermissionSet`, so an entry that
...
- **A child that writes a permission set does not inherit its parent's implementation
  choices.** §1 makes a map the whole statement of what a state holds and whose
  code serves it; a state that wants its parent's permission set and more says so with
...
- **What it is handed and what it answers.** One argument, `request`
  ...
  `cwd`, `state`, `task` and `permissionSet`. It answers `"allow"` or `"deny"` and nothing
...
- **`smart` is a shipped function** — a host function, registered beside the approval prompt
  ...
  and a permission set that writes it is refused, with the spelling that replaces it. The
...
- **Lowering, re-derived.** A function is `ask` wherever the gate reads a mode, and
  ...
  state that declares no permission set (command tools, and `show_artifact` where
...
- **Module approval.** A function is loaded as the one state it runs as
  ...
  permission set names and an unresolvable one is a lint error and refuses the start; an
...
### Amended 2026-09-23: the project policy block is dissolved, and each function's defaults live under `functions`

The person's rulings, in the Settings redesign: command rules belong to the `bash` tool of each
permission set, and each function's defaults live in one place, a layered `functions` block in
`settings.json`. So:

- **The permission set is the only authority over a line, besides the built-in floor.** `policy.rules`,
  `policy.default`, `policy.tools` and `policy.toolDefault` are gone, and with them the composition
  that put an authored rule ahead of the floor and let a permission set only tighten it. A part is judged by
  `.jaira/`, then the floor and the built-in asks, then the permission set, as §4 has it; with no permission set
  judging the line, what the built-ins do not name is allowed. `JairaPolicy` is `{ builtins?: boolean }`,
  read off `functions.bash.builtins`, and it is the one thing a project says above every permission set.
- **`functions`** holds `smart` (`model`, `prompt` — the top-level `smart` block, moved),
  `review_artifacts` (`publish`, from `policy.remote.publish`; `settleAfter`, from
  `integrations.review.settleAfter`; both in their old formats) and `bash` (`builtins`, from
  `policy.builtins`, default `true`). It is parsed strictly, carries every default, layers like every
  other block, and is one declared section of Settings. `policy`, `smart` and `integrations.review`
  are refused by name, saying where each went.
- **A one-shot migration runs at open**, before the configuration is read, in the app and the CLI
  alike (`migrateSettingsLayers`, called by `openProject` and `openSharedProject`), per layer: it
  moves the three values, writes each rule as a command line into every permission set the layer can name —
  its own files in place, a lower layer's as an override that keeps following it — carries a stricter
  `policy.default` onto each `bash` line and a `policy.toolDefault` onto a set with no `other`, and
  deletes `policy` and `smart`. It never adds a line that would hand a set a shell it did not offer, never
  replaces a line a set already names, and never widens an allow; what it cannot say exactly
  (`argIncludes`, a rule with no program, an earlier rule that hid a later one) it reports, with a copy
  of every file it changed in place under the layer's `system/logs/`. What it cannot preserve is listed
...
agents have — or people keep wanting two permission sets at once, which `$ref`
