...
3. **Revival.** Under a live parent, a child is live when it never terminated, or when it ended without success after the parent's last advancement, which is a transition taken or a new child entered. An element an adoption appended to the batch the parent entered last is not a new child: it is part of that entry, written after the fact, and does not advance the parent. A child that succeeded after that advancement is listed in the parent's `unanswered`, once per key. A superseded child is not loaded and still counts toward the next occurrence under its key. The root is live unless it ended with success.
...
| 7 | The elements of one fan-out batch share one occurrence and each carries its element; an adopted element appended to a batch does not advance its parent, so an element there that failed is still live | `persistence/test/load.test.ts` "gives the elements of one batch one occurrence, and each its element"; `persistence/test/adoptFanOut.test.ts` "an element appended to a batch the parent ran is not the parent moving on …" |
