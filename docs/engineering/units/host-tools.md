...
- `workflowTools.ts`: the eight WORKFLOW tools of [decision 0005](../decisions/0005-connect.md) §3 — `workflows`, `start`, `move`, `tasks`, `answer`, `hold`, `release`, `stop` — registered by `registerWorkflowTools`. They are the only tools here that reach nothing in the workspace: each validates the shape a model handed it and calls a `WorkflowToolHost`, which is `createWorkflowHost` in the app over `AppService`'s own operations. `noWorkflowHost` is the host for a caller that serves none (the CLI, a test registry): every name resolves and every call answers why it cannot be served there, so a state that holds one still loads and runs.
...
| 18 | A task the conversation did not start is refused by every gesture and by `answer` | `workflowTools.test.ts` (app) "refuses a task this conversation did not start, and says which it was", "refuses a request nobody is waiting on, and one that belongs to a task this conversation did not start" |
...
| A `start` is called while an engine holds the conversation's task | the version is written and the move is QUEUED on the session (`afterRun`), taken when that run ends well | none needed | `status: "queued"`, and the child enters when the run ends |
| That run is stopped or fails instead | the queued move is dropped: a stop is the later word about what should happen next | call `start` again | the version is there and nothing entered |
