...
- `workflowTools.ts`: the eight WORKFLOW tools of [decision 0005](../decisions/0005-connect.md) §3 — `list_workflows`, `start_task`, `move_task`, `list_tasks`, `answer_question`, `hold_task`, `release_task`, `stop_task` — registered by `registerWorkflowTools`. They are the only tools here that reach nothing in the workspace: each validates the shape a model handed it and calls a `WorkflowToolHost`, which is `createWorkflowHost` in the app over `AppService`'s own operations. `noWorkflowHost` is the host for a caller that serves none (the CLI, a test registry): every name resolves and every call answers why it cannot be served there, so a state that holds one still loads and runs.
...
- What a workflow tool actually DOES: the host's own operations — `connectTask` and `moveTask` ([adoption](adoption.md), `persistence/connect.ts`), the document generator ([workflow-snapshots](workflow-snapshots.md)), the hubs ([interaction-hub](interaction-hub.md)) and [task-lifecycle](task-lifecycle.md). `workflowTools.ts` implements none of them and is bound to one conversation by whoever registers it.
...
| 13 | The eight workflow tools are exactly the ones registered under that name, the two that only look are `readOnly`, and one is recognised bare or as an agent spells an injected one | `workflowTools.test.ts` (runtime) "are exactly the eight the standard list names, and each is registered", "is recognised by its bare name and as an agent spells an injected one" |
| 14 | A workflow tool never throws: a shape it cannot use is returned as `{error}` or `{ok: false}` and the host is not called | `workflowTools.test.ts` (runtime) the whole of "what is turned away before it gets there" |
| 15 | With no host the eight resolve and every call says why it cannot be served there | `workflowTools.test.ts` (runtime) "resolves every name and answers WHY each cannot be served, so a state that holds them still loads" |
| 16 | `answer_question` can never reach an approval: `list_tasks` does not list one, and naming one anyway is refused by component | `workflowTools.test.ts` (app) "can never reach an approval — the gate's own component decides, and the list it reads holds no approvals"; "says what was started here, where it stands, and what it is asking — and nothing of the rest of the project" |
| 17 | A call whose required inputs do not all bind writes nothing and answers `missing` plus the target's schema; the second call with the values takes them, recorded `asked` or `inferred` | `workflowTools.test.ts` (app) "asks for what nothing binds, writes nothing, and takes the value the second time — recorded as ASKED", "records a value the conversation worked out for itself as INFERRED, with its confidence" |
| 18 | A task the conversation did not start is refused by every gesture and by `answer_question` | `workflowTools.test.ts` (app) "refuses a task this conversation did not start, and says which it was", "refuses a request nobody is waiting on, and one that belongs to a task this conversation did not start" |
...
| A workflow tool is called where nothing serves it (the CLI, a test registry) | `noWorkflowHost` answers `{ok: false}` or `{error}` naming the place | run it in the app | the model reads the reason and says so |
| A `start_task` is called while an engine holds the conversation's task | the version is written and the move is QUEUED on the session (`afterRun`), taken when that run ends well | none needed | `status: "queued"`, and the child enters when the run ends |
| That run is stopped or fails instead | the queued move is dropped: a stop is the later word about what should happen next | call `start_task` again | the version is there and nothing entered |
