...
status: shipped
updated: 2026-09-22
implements: [product/bring-your-own-models-and-agents, product/work-inside-wsl, product/risky-actions-wait-for-approval, ui/surfaces/settings-connections, ui/surfaces/settings-models, ux/patterns/checked-status-with-the-fix]
layer: core
owns_contracts: []
...
verified_by: [packages/runtime/test/agents.test.ts, packages/runtime/test/agentToolPlan.test.ts, packages/runtime/test/codexBridge.e2e.test.ts, packages/runtime/test/genericAgent.test.ts, packages/runtime/test/executors.test.ts, packages/cli/test/genericAgent.e2e.test.ts]
...
- `agentTools.ts`: what each agent executor declares about its own tools ([0007](../decisions/0007-permissionSets.md) §3), as an `AgentToolDeclaration`: `channel`, how a permission set reaches the agent (`tools`, `switches` or `none`); `natives`, the agent's own tool names, each mapped to the standard tool it is or to `null` when no standard tool does that job; and `switches`, for a coarse transport, each switch with the standard subjects that turn it on. `CLAUDE_TOOLS` serves `claude-code` and `claude-cli`: channel `tools`, `Read` → `read_file`, `Glob` → `glob`, `Grep` → `grep`, `Edit`, `MultiEdit` and `NotebookEdit` → `edit`, `Write` → `write_file`, `Bash` → `bash`, `WebFetch` → `web_fetch`, `WebSearch` → `web_search`, and `Task`, `Agent` and `SlashCommand` → `null`. `CODEX_TOOLS`: channel `switches`, `shell` → `bash`, `apply_patch` → `edit`, and the switch `workspace-write` for `write_file`, `edit` and `bash`, which is on only where the permission set keeps one of codex's own writers; the tools a permission set holds are served to codex over its MCP bridge, each call gated there. `GENERIC_CLI_TOOLS`: channel `none` and no natives. `AGENT_TOOLS` and `agentToolsOf(name)` in `agents.ts` key them by registry name, and `nativeNamesByRoute(standard)` is what the composer is handed: claude's and codex's own name for a standard tool, since both can now be served ours or keep their own. Applying a declaration to a call is [tool-policy](tool-policy.md)'s: a state whose permission set is a map is held to it, and a state that declares no permission set reaches its agent as the executor built it.
...
- How an agent is driven: argv, stream parsing, sessions, the MCP permission and tool bridge and its worker. Those are upstream `@declarative-ai/agents-api` and `@declarative-ai/agents-cli`. JaiRA only creates one `createMcpBridgeHost` per `AppService` and per CLI process and passes its `start` as `startBridge` — to claude and to codex alike. Upstream points codex at it with a `-c mcp_servers.dai={…}` override measured against `codex-cli 0.147.0`: `required = true` (codex waits for the handshake, and fails the run past `startup_timeout_sec` = 30), `approval_mode = "approve"` on each served tool (under `auto` every call came back `user cancelled MCP tool call`), and `tool_timeout_sec` = 86400 for a call parked on a person; and, codex having no permission callback, `AgentExecutor` puts each call that crosses the bridge to `ctx.gate` before the tool runs.
- Agents reached by a model prefix, which `agentPromptRoutes` builds with the same spawn: [model-routing](model-routing.md).
- Compiling the policy, deciding a tool call, and holding an agent to a permission set with `planAgentTools` and `withAgentPermissionSet`: [tool-policy](tool-policy.md). The host tools the SDK adapter's built-ins are replaced by: [host-tools](host-tools.md).
...
- Layer `core`, package `@jaira/runtime`. It calls [process-exec](process-exec.md) for `resolveInvocation`, `NodeExec`, `killTree` and `detachedForTree`, [secret-chain](secret-chain.md) for probe credentials, and `claudeReplacements`, `agentFunctionWrapper` and `holdAgentFunction` from [tool-policy](tool-policy.md), which give the two claude functions `replacesNative` from the declaration and hold every agent function to its state's permission set: the claude functions by their route's own wrapper, passed to upstream as `wrapExecutor`, so a function call is handed exactly what the route is; codex read-only where the permission set leaves its writing switch off, served its held tools; and a generic CLI refused a permission set that denies anything.
...
| A prompt state reaches a generic CLI or `claude-cli` through a model prefix | `gateCapabilities` reads function ops only, so the start check does not examine it; a generic CLI route still refuses at the call when the state's permission set denies anything | none | the run starts |
...
| A codex or generic CLI agent is reached as a function rather than by model prefix | `holdAgentFunction` applies the declaration by the function's own channel: codex runs `permissionMode: "plan"` (its read-only sandbox) where the permission set leaves the writing switch off and is served the held tools over its bridge, and a generic CLI refuses a permission set that denies anything | none needed | the function op runs held to its permission set |
| Codex cannot finish the bridge's handshake within 30 s | the server is `required`, so codex refuses to start and exits 1 with `required MCP servers failed to initialize` | run the state again | the state fails |
| Codex's installed version reads the MCP override differently from 0.147.0 | measured against 0.147.0 only; a version that ignored `approval_mode = "approve"` would cancel every call again, and the agent would say so in its answer | re-measure with a real run against the new version, as the 2026-09-22 note of decision 0007 describes | the agent reports its tool calls cancelled |
| A state declares no permission set, or runs from a snapshot pinned before the list form was removed | no permission set marks on its block, so the call reaches the agent untouched: claude keeps every built-in under the gate, codex its configured sandbox | write a permission set on the state; start a pinned task again | the agent has its own tools |
