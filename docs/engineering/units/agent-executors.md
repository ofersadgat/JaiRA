...
| A codex or generic CLI agent is reached as a function rather than by model prefix | `holdAgentFunction` applies the declaration by the function's own channel: codex runs `permissionMode: "plan"` (its read-only sandbox) where the toolset leaves the writing switch off and is served the held tools over its bridge, and a generic CLI refuses a toolset that denies anything | none needed | the function op runs held to its toolset |
