...
- Layer `core`, package `@jaira/runtime`. It calls [process-exec](process-exec.md) for `resolveInvocation`, `NodeExec`, `killTree` and `detachedForTree`, [secret-chain](secret-chain.md) for probe credentials, and `claudeReplacements` and `agentServices` from [tool-policy](tool-policy.md), which give the two claude functions `replacesNative` from the declaration and remove the built-in of every standard tool a state does not hold.
...
| A codex or generic CLI agent is reached as a function rather than by model prefix | the declaration is not applied: the codex sandbox is the configured one and a denying toolset is not refused | reach the agent through a model prefix | the function op runs under the configured sandbox |
