...
| 7 | `models.default` is refused with the path it moved to, and an empty document parses to `defaultConfig()` | `shared.test.ts` "defaults artifactDir, and refuses the retired models.default", "points a config still carrying models.default at where those settings moved" |
...
| A layer sets a block to `null` | the null replaces the base's block; every block then fails parse except `artifactDir`, which reads as the default | remove the key | the error names the block |
...
- `artifactDir` still seeds `artifacts.dir` when `artifacts.dir` is absent.
