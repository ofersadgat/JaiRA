...
| 16 | `chat:startPlan` reads the state file, folds picks over it, and answers for a state that is not installed | `chatConversation.test.ts` `"reads them off the state file — what the first message would inherit, and from where"`, "folds a pick over the file, and says it was a pick", `"answers for a state nobody has installed yet — the view writes its files on the first send"` |
