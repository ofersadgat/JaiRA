...
| 15 | With nobody to ask, an `ask` is refused and never waited on, and the refusal names why and the setting that would allow it; `--approve deny` never asks; `--approve ask` with no terminal is a usage error before anything is made | `commandApproval.test.ts` "REFUSES, naming what would let the line run — the terminal, or the setting that asked", "refuses a BUILT-IN ask with the setting that would answer it, and runs nothing", "never asks under --approve deny", "refuses --approve ask with no terminal as a usage error, before anything is made" |
| 16 | A permission set hands claude and codex the same in a CLI run as in an app run | `commandApproval.test.ts` "claude is handed the same under %s", "codex is handed the same under %s, by a route and as a function" |
...
| A run meets an `ask` with nobody to answer | the approver refuses at once with a reason naming why nobody was asked and what to write; an agent is told no and goes on, `run_command` fails its state with that reason | answer at a terminal, or write the rule or permission set line named | `refused: …` on stderr; exit 1 if the state fails |
