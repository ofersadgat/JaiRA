...
- **Into a fan-out, the task is ONE ELEMENT** (the person's ruling, 2026-09-22). A child mounted `each: "inline"` or `"task"` adopts in the `element` shape (`AdoptedChild.shape`, with `each` and `index`):
  ...
  - *`"inline"`*: the engine loads a recorded inline element under the MOUNTED state, so a per-element stand-in is never consulted there. [run-load](run-load.md) folds an inline batch whose every element is adopted into ONE history row under the batch stand-in `jaira:adopted-batch:<stateId>`, its outputs gathered as `combineElements` gathers them (`gatherElementOutputs`). An inline batch the parent ran itself cannot take an adopted element and is refused.
...
- An inline batch of adopted elements loads under a **batch stand-in**, `jaira:adopted-batch:<stateId>`: every declared output as an array, fed the gathered outputs. It exists because upstream's `loadTerminatedFanOut` and `runFanOut` load a recorded element under `bundle.states[decl.state]` rather than under the element row's own `stateId`, as `loadTerminated` does for a single child. With that one change upstream, each element would be its own stand-in, a batch mixing run and adopted elements would load, and the fold and the batch stand-in would go.
...
| 16 | A child mounted `each: "inline"` or `"task"` adopts as one element: a batch it makes is the task alone and the list is `[element]`; a batch the parent has gets it appended at its length; the fan-in reads the batch; a value that does not fit the list's element type is refused naming both | `taskAdoptFanOut.test.ts` "in a NEW parent: the batch is the task alone …", "appends to a batch the parent HAS …", "makes the adopted task one of the batch's tasks …", "appends to its batch, and a list that holds more than the task places it …", "refuses a task whose element does not fit the list's element type …" |
| 16a | An inline batch of adopted elements loads as one row under the batch stand-in with its outputs gathered; a `"task"` batch keeps its rows for the host | `taskAdoptFanOut.test.ts` "loads the batch as ONE history row …"; `adoptFanOut.test.ts` "an inline batch of adopted elements is ONE row …" |
| 16b | A mount over two lists, an inline batch the parent ran itself, a batch the parent has entered something after, a supplied list an inline task is not alone in, and a parent that already split are refused | `adoptFanOut.test.ts` "which batches take an element"; `taskAdoptFanOut.test.ts` "refuses a supplied list …", "refuses a parent that has already split …" |
...
- An inline batch of adopted elements reaches the engine as a single child where the journal holds one row per element. The projection and the conversation read the journal and still see the elements.
