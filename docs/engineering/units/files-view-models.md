...
verified_by: [packages/persistence/test/stateViews.test.ts, packages/persistence/test/fileLint.test.ts, packages/app/test/service.test.ts, packages/app/test/eachHosted.test.ts]
...
- `fileTree(project, browser?, hidden?)`: the project root walked from the checkout with its layer prefix measured, then every later layer root, each filtered by the compiled hidden rules. A state file gets its `stateId`, a base file the project overrides is marked `shadowed`, an unparsable file carries `error`, and lint counts are joined from the browser and rolled into directories. `baseFileTree(baseDir, browser?, hidden?)` is the shared root alone.
