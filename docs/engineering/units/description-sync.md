...
- `syncWorkflow.ts`: `workflow/sync/document` mounts both check states and then `revision`, which returns the rewritten description as a plain string. `workflow/sync/states` mounts them and then `edits`, which returns whole files under `workflows/` or `prompts/`. Every state inherits `tools: ["read_file"]` and `permissions: {profile: "read-only", tools: {read_file: "allow"}}`. `syncOutcomeOf` parses a run's outputs, `syncRespondPrompt(spec)` is the review loop's respond prompt with `{{` in the description widened to `{ {`, and `syncRules` and `conformanceRules` script fake runs by matching prompt sentences.
...
- `startRun` registers its own file, search and web tools after `syncCapabilities`, so the registry a sync runs with holds more than `read_file`. What bounds a sync state is its `tools` list and the `read-only` profile.
- Upstream seams: `@declarative-ai/hw` `loadBundle` loads each workflow map; the engine runs it through `startRun`; the engine and each agent transport enforce the `read-only` profile.
...
| 11 | Every sync state is authored `read-only` with `read_file` as its only tool | `syncWorkflow.test.ts` "authors the read-only contract where the engine can enforce it, on every state" |
