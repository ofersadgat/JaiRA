...
| 11 | Every sync state's toolset holds `read_file`, `glob` and `grep` as `allow`, refuses `edit`, `write_file` and `bash`, answers `deny` for `other`, and carries no profile | `syncWorkflow.test.ts` "authors the read-only contract as the toolset, on every state, and hands the engine no profile" |
