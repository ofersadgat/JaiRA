...
- Building the agent executors a node selects: [agent-executors](agent-executors.md) and `agentPromptRoutes` in [model-routing](model-routing.md). Each arrives already wrapped in `withAgentPermissionSet` with that agent's own tool declaration, so the executor an `agent` node selects passes a legacy list-form run through untouched and, under a permission set map, removes the built-ins the map does not hold, sets codex's sandbox from it, and refuses a denying permission set on a generic CLI: [tool-policy](tool-policy.md).
...
- A permission set is applied at the leaf and not in the tree. The floor, a router's defaults and a node's model rules all run before the route is known or without caring which it is; which built-ins to remove is a fact about the one agent that answers, so `withAgentPermissionSet` sits innermost, around the agent executor itself, and the ask rules it writes are unioned into the same `providerOptions` permission arrays the floor writes.
