...
- **What it runs.** A numbered list of the two states, each a state id in the data face followed straight on, with no space, by an app sentence in `--dim` whose value names are in the data face. Under it a disclosure in `--dim`, `The state files, as they will be written`, which opens to each file's name over its JSON in a box on `--bg` with a 1px `--line` edge and 8px corners.
- **Installed.** An accent-washed notice saying why the self-test lives in the shared root. Then one row per state file: a 7px dot, the state id in the data face, a hairline pill `installed`, `edited` or `missing`, and `open ↗` in `--accent`; pointing at a row gives it a `--panel-2` ground. Under the rows the first file's absolute path in `--dim`, then three ghost buttons `Install missing`, `Reinstall` and `Re-check`.
...
| Captions | `What it runs` · `Installed` · `Run it` · `Result` · `What was actually said` · `Journal` |
...
| Disclosure | `The state files, as they will be written`; each file named by its state id and extension |
| Installed notice | `The self-test lives in the SHARED root, not in the open project — it is a fact about this installation, and three debug files in a checkout's .jaira/ would be three files in its next commit.` |
| File rows | `installed` · `edited` · `missing` · `open ↗` |
| Edited notice | `One file differs from what this build would write. Left alone — a run uses what is on disk. Reinstall to put them back.`, or `{n} files differ` for several |
| Buttons | `Install missing` · `Reinstall` · `Re-check` · `Run (live LLM)` · `Run scripted` · `New task` · `Cancel` |
...
| --- | --- | --- |
| first_run | Nothing run from this window: the four steps with every file `installed` on a green dot, `Install missing`, `New task` and `Cancel` at half opacity, a readiness notice, and `Run the self-test to see its task here.` on the right. | [first-run.html](../assets/debug-view/first-run.html) |
| warned | Whatever stands in the way, said where it applies. An `edited` file on a `--dim` dot adds the amber edited notice; a `missing` file on a `--bad` dot enables `Install missing` and disables its own `open ↗`. With no check reported, or none healthy, the readiness notice is amber. Standing at the root with no project, the amber no-project notice appears and every run button is at half opacity. A failed install or run adds its message on a `--bad` wash under the buttons. | [warned.html](../assets/debug-view/warned.html) |
| loading | Looks like running with fewer parts: while files are written or a run is being started, the install, reinstall, re-check and run buttons are at half opacity, and `Started — no run has settled yet.` in `--dim` stands under `Result` in place of the band until the run's first record arrives. | [running.html](../assets/debug-view/running.html) |
...
- `Run (live LLM)` writes any missing file, never overwriting an edited one, then runs again on the last self-test task, or creates `Self-test (live)` in the shared root when there is none. `Run scripted` does the same with canned replies, and a task it creates is `Self-test (scripted)`. `New task` always creates a fresh task.
- `Install missing` writes only the missing files and `Reinstall` writes all three over any edit; `Re-check` reads their status again. A failure notice goes away when it is clicked.
- `Cancel` stops the self-test's task.
- `open ↗` on a file row, or `Open state ↗` in the task panel, leaves for the [files view](files-view.md) with that state open in the shared root.
...
- **Resize.** The right column stays 360px and the page takes the rest. Notices and the heading's paragraph wrap; the path line ends in an ellipsis and shows in full as its tooltip. The calls box stays 320px tall and both of its halves scroll on their own.
- **Theme.** Washes, bands, dots and boxes are tokens with a light and a dark value.
- **Focus.** Entering moves focus nowhere. Tab follows the page: the disclosure, each `open ↗`, the install buttons, the run buttons, the failure notice, then the task panel's buttons. A button at half opacity takes no focus. The state rows in the calls box take a pointer only.
...
- The installed path, the file names in the disclosure, the state ids in the calls box and the task title in the journal caption are data set in the app face.
