...
| adopted | `adopted` and the task's title and standing, led by a fold chevron instead of the glyph; under it, OPEN, the adopted task's own history — its steps, its sessions and their transcripts, read from that task — indented behind a 2px `--accent`-tinted edge, with no composer and no cut of its own. The conversation simply grows where the adoption happened. Folding it leaves the line. | [adopted-history.html](../assets/run-step-note/adopted-history.html) |
| asked | A MOVE that needed inputs asked for them here, in the task's own conversation, at the point it was made — WITH THE QUESTION UI and nothing around it: the stepped [choose-option-gate](choose-option-gate.md) an agent's batch of questions also uses, under the heading `Moving '{task}' to {target} needs {n} inputs.`, one step per input — its name in the pill, its declared description as the question. A string, number, list or object input is answered in the own-answer box; `Confirm` on the last step takes the move. | [asked.html](../assets/run-step-note/asked.html) |
| asked, enum or boolean | An `enum` input's step offers its values as the options; a `boolean` offers `Yes` and `No`. | [asked-enum.html](../assets/run-step-note/asked-enum.html), [asked-boolean.html](../assets/run-step-note/asked-boolean.html) |
| refused in place | An own answer the input's schema cannot read, or refuses, is said under the step in `--bad` (`expects a number`, `must be ≥ 1`) and the step button stays disabled; the question stays where it is. | [asked-refused-in-place.html](../assets/run-step-note/asked-refused-in-place.html) |
| answered | The question as it was answered, as a settled question is drawn: the picks lit, the words as typed, `Back` and `Next` to page through it. A move refused when it came to be taken says so in the question's own error line, `The move could not be taken: {reason}` in `--bad`; a question a stop or a rewind withdrew reads `Never answered.` | [asked-answered.html](../assets/run-step-note/asked-answered.html), [asked-refused.html](../assets/run-step-note/asked-refused.html) |
