...
mockups: [ui/assets/run-step-note/entered.html, ui/assets/run-step-note/failed.html, ui/assets/run-step-note/made.html, ui/assets/run-step-note/rewind-armed.html, ui/assets/run-step-note/skipped.html, ui/assets/run-step-note/moved.html, ui/assets/run-step-note/asked.html, ui/assets/run-step-note/asked-answered.html, ui/assets/run-step-note/adopted-history.html]
...
| asked | A MOVE that needed inputs asked for them here, in the task's own conversation, at the point it was made: `asked for the move to **{target}** · {inputs}` in `--dim`, and under it the question itself — the fill form gate over the inputs' own schemas, live, whose `Submit` takes the move. | [asked.html](../assets/run-step-note/asked.html) |
| answered | The same row once the question is done, verb `answered`: the form drawn as it was answered, then one line — `Answered — the move to {target} was taken.`, or in `--bad` `Answered, and the move could not be taken: {reason}`. A question a stop or a rewind withdrew reads `No longer open — the move was not taken.` | [asked-answered.html](../assets/run-step-note/asked-answered.html) |
