...
mockups: [ui/assets/run-step-note/entered.html, ui/assets/run-step-note/failed.html, ui/assets/run-step-note/made.html, ui/assets/run-step-note/rewind-armed.html]
...
**Use when.** A run entered a state: every entry gets a note, whether or not the state then spoke. A state could not be entered, or gave up, with no conversation to report it. A fan-out turned its elements into tasks of their own: the note names and links them.
...
- **Verb.** `entered`, `could not enter`, `made` or `split off`, in `--dim`. A failure with no verb starts with its path.
...
| Where | String |
| --- | --- |
| Verbs | `entered` · `could not enter` · `made` · `split off` |
