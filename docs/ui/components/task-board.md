...
| asking | A column whose target needs an input nothing the task produced gives lights like any other: the move is decided, and asks for the input in the task's OWN conversation, where it is answered through the schema form before the move is taken. The preview names each input among the facts as `{input} will be asked in this task's conversation` with its description, and its drop line ends `then answer in its conversation`. After the drop the task is selected, so its conversation, with the question in it, is in view. | [asking.html](../assets/task-board/asking.html) |
| refused | A column the move table refuses — a state the task cannot reach from where it stands through anything its process defines, a sideways jump or a branch whose decision it already took — stays unlit and refuses the drop. Under the pointer it holds the preview, kind `Cannot move here`, the host's sentence naming why, and `The workflow does not go there from where this task stands.` in `--dim` where the drop line would be. A column refused for any other reason says that reason the same way. | [refused.html](../assets/task-board/refused.html) |
| confirming | A move of a WORKING task back to a state it entered, or into another process, lights like any other, and its drop line reads `Drop, then confirm: stop it and go back` or `Drop, then confirm: pause it and move it`. Dropped, the question stands in the column the task would land in, under its cards, in the preview's box with a `--warn` edge and two buttons: the filled yes (`Stop and go back`, `Pause and move`) and `Cancel`. Yes is the commit; nothing moves before it. A task only waiting for the person, or for an event, is moved without the question. | [confirming.html](../assets/task-board/confirming.html) |
| after a drop | The drop has happened; nothing was confirmed. A task the drop made stands in its column with `Undo` at the far end of its second line, and the task it adopted is drawn beneath it. `Undo` stays through a restart and while the run the drop started is going, and goes as soon as the task does anything past where the drop put it, or anything else is decided about it. | [after-drop.html](../assets/task-board/after-drop.html) |
...
| Drop on a column that asks | Makes the conversation that controls the work, without the move; its first turn asks for what the target needs, and the move is taken when the conversation starts the target with the answer | The board redraws; the task the drop made or gave a conversation carries `Undo`, and the conversation is a row in the Chat list |
| Click `Undo` | Takes the drop back, stopping the run the drop started if it is still going: an adopted task is its own again and a task the drop had made is removed, its conversation with it; a moved task returns to where it stood; a task given a conversation loses it | The cards return. `Undo` is only drawn while the drop is the last thing the task did: once the state it landed in finishes, anything after it starts, a conversation that asked starts the target, or the task is moved, stopped, rewound, forked or run again, the card no longer carries it and the task's rewind takes the drop back |
...
| Preview, adopt into a fan-out | `{workflow} runs {child key} once per element of a list.` (`…, each as a task.` for `each: "task"`) `The task becomes one of those elements; nothing runs again.` (`…tasks…`) · facts `the batch is this task alone` · `added to its batch as element {n}` · `element {n} of the list; the rest are made as tasks` (`…; the rest run here` for `each: "inline"`) · then the adopt facts |
...
| Preview, asks after | `No workflow holds both {own} and {target}.` then `A new one is made around this task, with a conversation to control it from, which asks before the move is taken.` or `This task's copy of the workflow gains a conversation to control it from, which asks before the move is taken.` · `This task's conversation asks for what {target} needs, then takes the move.` · facts `this task becomes its first child` · `the copy stops following {own}` · `{input} will be asked there` with ` — {description}` when the input has one · drop `Drop to open the conversation` · `Drop to ask in the conversation` |
...
| Preview, refused | `{states} lie between` with the host's reason · `{n} required inputs would not be bound.` with `{input} is required and unbound — {reason}`, for a move within a process or an adoption · `could join {workflow} as {child key}` when more than one process holds both · otherwise the reason as the host gave it |
