...
mockups: [ui/assets/task-board/empty.html, ui/assets/task-board/root-listing.html, ui/assets/task-board/level.html, ui/assets/task-board/dragging.html, ui/assets/task-board/connecting.html, ui/assets/task-board/asking.html, ui/assets/task-board/refused.html, ui/assets/task-board/after-drop.html, ui/assets/task-board/run-board.html]
...
| asking | A column whose target needs an input nothing the task produced gives, where the move adds to the workflow, lights like any other: the drop makes, or uses, the conversation that controls the work, and that conversation asks. The preview says so in its sentence, names each input among the facts as `{input} will be asked there` with its description, and its drop line says the conversation opens. Nothing is refused and nothing is confirmed. | [asking.html](../assets/task-board/asking.html) |
| refused | A column a drop would do nothing in stays unlit and refuses the drop. Under the pointer it holds the same preview with the reason in `--dim` where the drop line would be. A move within the task's process or an adoption that would leave a required input unbound is still refused, each such input named among the facts. | [refused.html](../assets/task-board/refused.html) |
