...
| Click `Undo` | Takes the move back: an adopted task is its own again and a task the drop had made, having run nothing, is removed, its conversation with it; a moved task returns to where it stood; a task given a conversation loses it | The cards return; a refusal while the task runs says to stop it first |
