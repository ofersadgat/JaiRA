...
implemented_by: [packages/app/src/renderer/composer.tsx, packages/app/src/renderer/composerToolset.ts, packages/shared/src/toolsetBuckets.ts]
verified_by: [packages/app/test/composer.test.ts, packages/shared/test/toolsetBuckets.test.ts]
...
| resting | Four dim chips. A value chosen for this message turns its chip `--accent`. Permissions reads the name of the toolset the tools and modes exactly match in the card's bucket, and `custom` when they match none. A call that declares no tools reads what decides instead, `ask by default`. Drawn: inherited, chosen here, nothing declared, and loading. | [resting.html](../assets/composer-setting-chip/resting.html) |
...
| A Permissions row | Writes that toolset's ticks, implementations and modes onto the Tools card for this message | The row ticked and the chip reading its name; the Tools card shows the new lines |
...
| `+` in the Permissions head | Opens a name and where it goes; `Add` keeps the tools and modes as a new toolset in the bucket, in this project or for all projects | The form closes, the toolset is a row of the bucket and is ticked, and `+` dims. A name that cannot be a file, or one the layer already holds, is said under the form |
...
| A command's button | Takes the command out of the toolset | The line goes; the program's count drops |
| `script`'s button | Names running a file in the toolset, or stops naming it | Tick and wash appear or go |
...
| Chip values | `{route}/{model}` or `no model configured` · `low` `medium` `high` `xhigh` or `the model's default` · `{toolset}`, which for what ships is `ask first` `read-only` `auto` `full access`, or `custom`, or `{mode} by default` when no tools are declared · `{n} tools` · `{tool}` · `no tools` · `default tools` |
...
| Bucket picker | `{bucket}` `built in` · `all projects` · `this project` over `{toolset} · {toolset} · …`, or `{n} toolsets, and {n} buckets inside`; `chat_control` reads `its own four — the task and workflow tools only` · tooltip `Which bucket of toolsets these rows are` |
| `+` | tooltip `Keep these tools and modes as a new toolset in {bucket}` · `These tools and modes are already a toolset here` · form `Keep these tools and modes as a toolset in {bucket}` · `name` · `where` `in this project` `for all projects` · `Add` |
| Permissions sentence | `A toolset: which tools are offered, and what happens when each is called. Change any of it under Tools and this becomes custom.` · `These tools and modes match no toolset in {bucket} — + keeps them as a new one.` · `No toolsets in {bucket} yet. + keeps the tools and modes under Tools as the first.` |
