...
A block under a running agent's words, below a blue rule: a shield glyph and `Approve this command?`, the tool's name in small grey type, the command in a bordered mono box with each of its requests tinted in its own colour, one row per request under it, an amber line naming the permission set and the subject that asks, and two split buttons, a filled `Allow` and a red-outlined `Deny`, each with an arrow.

## A command approval asks about one tool call that policy held back

**Use when.** An agent's tool call resolved to ask: a shell line with a part the state's permission set asks about, a command a policy rule or a built-in holds back, or a write the policy wants a person to see. The agent waits for a person to allow or deny it.
...
| menu open | One menu at a time, under `Allow` or under `Deny`. A built-in or shared permission set offers `creates {file} with {line}, following the built-in permission set`. A permission set written on the state offers once and this run, and says why under a rule. A tool with no command line offers once and this run. | [answer-menu.html](../assets/command-approval/answer-menu.html) |
...
- **Answers.** `Allow` and `Deny`, plain buttons with no menu: what a function asked is never remembered or written into a permission set. Answered, the pill reads `allowed` or `denied` and the chosen button stays lit.
...
## The copy names the tool, the requests, the permission set and the line to be written
...
| Reason | `Permission set {bucket/name}: {entry} asks` · `This state's permission set: {entry}, {entry} and {entry} ask` · `Permission set {name} holds no line for {subject}, so it asks` · `Policy: {reason}` |
| Answers | `Allow` · `Deny` |
...
| Menu, what is written | `writes "{subject}": "{allow or deny}" to {file}` · `creates {file} with "{subject}": "{mode}", following the built-in permission set` or `the shared permission set` · `— this project's own copy still wins here` |
| Menu, why not | `This permission set is written on the state itself, so there is no permission set file to add a line to.` · `This line could not be read well enough to remember an answer about it, so it is asked about every time.` · `Part of this line could not be read; that part is asked about every time.` |
...
- The menu is at most 460px or 86% of the window wide; its sentences wrap, and a file path is shortened to its ends, `.jaira/permission-sets/…/writes-asking.json`.
...
- Subjects, permission set names, widths and the line to be written are data and are set in the data face, inside sentences in the app face.
