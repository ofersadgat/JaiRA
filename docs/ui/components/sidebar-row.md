...
- **Drawer.** Under the current row, when its room has something to browse: indented 6px with a 1px `--line` rule down its left and 8px inside it. Files opens the file tree, Chat and All conversations open the conversation list, Settings opens its sections. Tasks, All tasks, Logs, Debug and Components have none.
...
| lifted | Settings, while it or Logs, Debug or Components is showing: a panel covers the column under the title row, headed by the Settings row with a `‹` before its glyph and its sections beneath; Logs, Debug, Components and the theme row sit at the panel's foot under a rule. | [lifted.html](../assets/sidebar-row/lifted.html) |
...
| Settings sections | `Providers` · `Executors` · `Configuration` · `Files` · `Appearance` · `Conversation` · `History`, the last only inside a project |
